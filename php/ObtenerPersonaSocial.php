...
if($VerificarEncuesta->RowCount()>0){
    ...
    foreach ($VerificarGrupo as $Integrante){
        $dni_titular=$Integrante['dni_titular'];
    }
    $ObtenerEncuesta = $conexion -> prepare ("SELECT * from personas WHERE dni=:dni");
    $ObtenerEncuesta -> bindParam(':dni',$dni_titular);
    $ObtenerEncuesta -> execute();
    foreach($ObtenerEncuesta as $persona) {
        $dni=$persona['dni'];
        $nombre=$persona['nombre'];
        $estado_civil=$persona['estado_civil'];
        $fnacimiento=$persona['fnacimiento'];
